# Security Policy for `{{ cookiecutter.project_slug }}`
...
For maximum isolation, run `{{ cookiecutter.project_slug }}` inside a container so it only has access to what you
explicitly mount.
...
docker run --rm -v "$(pwd):/data" {{ cookiecutter.project_slug }} /data
```
