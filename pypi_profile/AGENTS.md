...
> **This is a sub-package of a uv workspace.**
> Run `uv run` from the **repo root** (`matthewdeanmartin_pypi/`) or from this directory.
> Never use bare `python`, `pip`, or `pytest` — the system Python lacks the project dependencies.
> See the root `AGENTS.md` for full workspace guidance.

## Name collision warning

`python -m pypi_profile` fails because Python resolves the `pypi_profile/` *directory* before the
installed package. Use the entry point:
