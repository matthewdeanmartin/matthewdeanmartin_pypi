...
def _cache_write(key: str, payload: Any) -> None:
    p = _cache_path(key)
    p.write_text(json.dumps({"_ts": time.time(), "payload": payload}), encoding="utf-8")
...
def fetch_all(profile: ProfileData, verbose: bool = False) -> dict[str, Any]:
    ...
    for link in profile.profiles:
        if link.kind == "github" and link.url:
            gh_username = _extract_github_username(link.url)
            if gh_username:
                ...
                if cached is not None:
                    ...
                    _cache_write(key, github_profile)
    ...
    for link in profile.profiles:
        if link.kind == "gitlab" and link.url:
            gl_username = _extract_gitlab_username(link.url)
            if gl_username:
