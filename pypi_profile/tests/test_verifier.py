...
from pypi_profile.models import ProfileData, IdentitySection, VerificationSection, ProfileLink
...
def test_fetch_page_urllib_fallback(mocker: Any) -> None:
    ...
    mocker.patch("builtins.__import__", side_effect=lambda name, *args, **kwargs: (
...
def test_verify_profile_link_invalid_signature(mocker: Any) -> None:
    mocker.patch("pypi_profile.verifier.fetch_page", return_value="pypi-profile-proof: valid_token")
    mocker.patch("pypi_profile.verifier.decode_claim", return_value={
...
def test_verify_all_profiles(mocker: Any) -> None:
    ...
    def mock_verify(link, **kwargs):
        if "gh.com" in link.url: return "verified"
        return "unverified"
