...
def test_summary_page_renders(client) -> None:
    response = client.get("/")
    assert response.status_code == 200
    assert "John Doe" in response.text


def test_packages_page_renders(client) -> None:
...
def test_hiring_page_renders(client) -> None:
    response = client.get("/hiring")
    assert response.status_code == 200
    assert "Consulting" in response.text


def test_contact_page_renders(client) -> None:
    response = client.get("/contact")
    assert response.status_code == 200


def test_verification_page_renders(client) -> None:
    response = client.get("/verification")
    assert response.status_code == 200
    assert "PyPI-published" in response.text


def test_succession_page_renders(client) -> None:
    response = client.get("/succession")
    assert response.status_code == 200
    assert "Succession policy" in response.text


def test_api_profile_json(client) -> None:
...
def test_api_packages_json(client) -> None:
...
def test_api_people_json(client) -> None:
